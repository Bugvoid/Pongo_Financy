...
import boto3
import tempfile
from dados.dao_download_cra import DAO_DownloadCRA
...
# https://fnet.bmfbovespa.com.br/fnet/publico/pesquisarGerenciadorDocumentosDados?d=5&s=0&l=10&o[0][dataEntrega]=desc&tipoFundo=5&idCategoriaDocumento=6&idTipoDocumento=86&idEspecieDocumento=0&cnpj=BRBZRSCRA0C9&dataInicial=01/01/2000&dataFinal=27/01/2020&paginaCertificados=true&_=1649685983701
...
def create_parser():
    ...
    parse.add_argument(
        ...
        required=True,
        type=str,
        help='Define prefixo do arquivo a ser salvo no download. Ex: CRA = CRA-BRHBSCCRA0O3-20200131-20200128-114809-110294.xml'
    )
    ...
    parse.add_argument(
        ...
        type=int,
        default=81,
        help='Id do tipo documento do campo em "extrair inf. mensal" do fnet. 81=informe mensal de CRA (Anexo 32, II ICVM 480)'
    )
    ...
    parse.add_argument(
        "-ident", "--identificador",  # exemplo: BRHBSCCRA0O3
        required=False,
...
def _get_data_de(args):
    ...
    if data_de_cache is None:
        ...
        dao = DAO_DownloadCRA('cra_download.db')
        ...
        try:
            if not data_de:
                dt_download = dao.get_dt_ultimo_download()

                if dt_download:
                    data_de = datetime.datetime.strptime(dt_download, "%Y-%m-%d")
                    logging.info(f'assumindo "data_de" a partir de {data_de}')
                else:
                    logging.error(
                        'Nenhuma data de processamento anterior encontrada, deverá ser informado uma data inicial ou um range de datas')
                    return False, None
            ...
            logging.error('32054 - parameter data_de with value invalid: "%s"',
                          args.data_de if args.data_de else data_de)
            return False, None
...
def _get_data_ate(args):
    ...
    if data_ate_cache is None:
        ...
        try:
            ...
            logging.error('32055 - parameter data_ate with value invalid: "%s"', args.data_ate)
            return False, None
...
def validate_args(args):
    ...
    if (args.output_path) and ((not os.path.exists(args.output_path)) or (not os.path.isdir(args.output_path))):
        logging.error('32056 - parâmetro output directory "%s" not exist', args.output_path)
        return False  # Erro
    elif (not args.output_path):
        if (not args.bucket_name):
            logging.error('32057 - parâmetro bucket_name não informado')
            return False  # Erro
        if (not args.bucket_prefix):
            logging.error('32058 - parâmetro bucket_prefix não informado')
            return False  # Erro

    if (not args.file_prefix) or (not args.file_prefix.strip()):
        logging.error(f'32059 - file_prefix é inválido "{args.file_prefix}".')
...
def nome_arquivo(tipo_fundo, identificador, data_ref, data_pub, extensao):
    if extensao not in ['pdf', 'xml']:
        extensao = 'xml'
    '''
        # formato original: <carga format-output-fileName="CRA-%1$tY%1$tm%1$td-%1$tk%1$tM%1$tS.cra">
        # Atual:
        # Formato: CRA-BRRGSSCRA043-20200131-20200128-114809-110294.xml
        # Formato: Tipo-identificador-YYYYMMDD-YYYYMMDD-HMS-IdLink
    '''
    return '-'.join([
        tipo_fundo,
        identificador,
...
def get_json_download(args, identificador):
        ...
        while continua:
            ...
            if resp_json.status_code == 200:
                ...
                if ((recordsFiltered == 0) or (recordsTotal == 0)):
                    continua = False
                    logging.warning(
                        f'Pesquisa não retornou dados para o filtro: args {args} e identificador {identificador}')

                if msg:
                    continua = False
                    logging.warning(
                        f'Pagina retornou a mensagem {msg} para o filtro: args {args} e identificador {identificador}')

                if not data:
                    continua = False
                    logging.warning(
                        f'Pagina não retornou dados para o filtro: {args} e identificador {identificador}')
                ...
                continua = False
                logging.error(
                    f'Erro ao pegar json da paginação atual. request: {json_request}, args: {args} e identificador {identificador}')
    ...
    if not result['data'] and len(result['data']) > 0:
        logging.warning(
            f'Pesquisa não encontrou dados para argumentos: {args} , identificador {identificador}, start {start}.')
    else:
        ...
        if lista_para_download['data'] and len(lista_para_download['data']) > 0:
            ...
            for download_item in lista_para_download['data']:
                ...
                if (args.limit_download != 0) and (num_download >= args.limit_download):
                    logging.warning(
                        f'numero maximo de download: {num_download}/{args.limit_download} execute novamente para continuar')
                    break
                ...
                if not (response is None):
                    if (response.status_code == 200):
                        ...
                        if (len(download_item['dataReferencia']) > 7):
                            ...
                            data_ref = date_with_last_day_in_month(
                                datetime.datetime.strptime(download_item['dataReferencia'], '%m/%Y')
                            )
...
def date_with_last_day_in_month(any_date):
